...
//{storeId: curEdge.store_id, productId: curEdge.product_id, price: curEdge.price}
...
Graph.prototype.oneStopSearch = function (shoppingList) {
    ...
    if(possibleStores.length === 0) {
        // what to do here? send what products we can get at one store for the best price?? or just simply route to a two stop search?
        // what to do here? send what products we can get at one store for the best price?? or just simply route to a two stop search?
        // what to do here? send what products we can get at one store for the best price?? or just simply route to a two stop search?
        // what to do here? send what products we can get at one store for the best price?? or just simply route to a two stop search?
        // what to do here? send what products we can get at one store for the best price?? or just simply route to a two stop search?
